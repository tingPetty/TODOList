...
def set_enabled(enabled: bool) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, f"操作启动项失败: {error}"
    return True, ""


def ensure_current_command() -> tuple[bool, str]:
    """Repair a stale/missing Run entry while autostart is enabled."""
    if winreg is None:
        return False, "当前平台不支持开机自启。"
    ...
    try:
        with winreg.CreateKeyEx(
            ...
            try:
                ...
                current = None
            if current != expected:
                winreg.SetValueEx(key, RUN_VALUE_NAME, 0, winreg.REG_SZ, expected)
    except OSError as error:
        return False, f"同步启动项失败: {error}"
    return True, ""
